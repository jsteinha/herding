...
each atom can be operated on efficiently, which leads to, for example, 
...
\begin{theorem}
\label{thm:main}
Given vector spaces $V$ and $W$\fTBD{JHH: hmm...you've switched from $\reals^{d}$ to vector spaces}, let $f : V \times W \to \reals$ be a function 
such that 
\begin{enumerate}
\item $f(\cdot,w)$ is linear for each fixed $w$
\item $f(v, \cdot)$ is strongly convex\fTBD{JHH: we haven't defined strongly convex yet} with respect to a norm $\|\cdot\|$ 
      for each fixed $v$.
\end{enumerate}
...
of $\|\cdot\|$.
\end{theorem}
This result is tight in the following sense:\fTBD{JHH: added complementary lower bound result}
\begin{theorem}
...
\begin{enumerate}
\item Intro
\item Sparsity result and lower bound
\item BMD and proofs of sparsity result and lower bound
\item Application: Low-rank matrix approximation, approximation of SDP matrix sums 
\item Application: Graph sparsification, max flow
\item Application: Solving games 
\item Application: Herding, including Chen extension
\item Application: Circuits and boosting
