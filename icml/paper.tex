...
\[ L(u) = \max_{\theta \in \Theta} [\langle \theta, u \rangle - R(\theta)] \]
$R$ is some convex function that we should think of as a measure of complexity 
of $\theta$. Thus, the loss is equal to the maximum amount that $\theta$ can match 
$u$, regularized by the complexity function $R(\theta)$.

Examples:
\begin{itemize}
\item Let $R(\theta) = \langle \theta, u_0 \rangle$. Then $L(u)$ is the 
...
bound:
\[ \sup_{\theta \in \Theta} L(\hat{u}, \theta) \leq \sup_{\theta \in \Theta} L(u^*, \theta) + O(1/\sqrt{T}). \]
