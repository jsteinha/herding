...
In this document we consider a family of methods for minimizing $L(u)$, 
...
throughout this document that $h$ and $R$ are both convex functions. \NA{JH: Should we comment on the symmetry introduced by this formulation?}
...
\[ \sup_{\theta \in \Theta} L(\hat{u}, \theta) \leq \sup_{\theta \in \Theta} L(u^*, \theta) + O(\log(T)/T). \]
\NA{JH: It would be more elegant to argue either here or below that the bounds on the second algorithm follow directly from duality}
...
and $u_{t+1} \in \arg\min_u h(u) + \langle \frac{1}{t} \sum_{s \leq t} \theta_s, u \rangle$. 
