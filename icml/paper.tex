...
Let $U$ and $\Theta$ be convex subsets of two Hilbert spaces $\sH_{U}$ and $\sH_{\Theta}$ and 
let $i : U \to \Theta$ be a linear map from 
$U$ to $\Theta$ that preserves the inner product. Consider the following loss function 
$L : U \to \bR$:
\begin{equation}
L(u) = h(u) + \max_{\theta \in \Theta} \left\{\langle \theta, i(u) \rangle - R(\theta) \right\}.
\end{equation}
...
\item Let $U$ be the probability simplex over a space $\sX$, let $\phi : \sX \to \bR^d$ be a collection 
      ...
      $\langle \theta, i(u) \rangle = \bE_u[\theta^T\phi(x)]$, and 
      $R(\theta) = \langle \theta, i(u_0) \rangle$. Then $L(u)$ is the 
...
\begin{equation}
L(u,\theta) = h(u) + \langle \theta, i(u) \rangle - R(\theta).
\end{equation}
We will assume throughout that $h$ and $R$ are both convex functions, 
and furthermore that $\arg\max_{u} h(u) + \langle \theta, i(u) \rangle$ can be efficiently 
...
\item $\theta_{t} \in \arg\max_{\theta \in \Theta} \langle \theta, i(u_t) \rangle - R(\theta)$
\item $u_{t+1} \in \arg\min_{u} h(u) + \langle \hat{\theta}_t, i(u) \rangle$
...
\item $u_t \in \arg\min_{u} h(u) + \langle \theta_t, i(u) \rangle$
%\item $\theta_{t+1} \in \arg\max_{\theta \in \Theta} \langle \theta, \frac{1}{t} \sum_{s \leq t} u_s \rangle - R(\theta)$
\item $\theta_{t+1} \in \arg\max_{\theta \in \Theta} \langle \theta, i(\hat{u}_t) \rangle - R(\theta)$
\end{enumerate}
...
$\langle \theta, i(u) \rangle$ when the map $i$ is clear from context.
