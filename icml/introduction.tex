...
For instance, Chen et al showed that the moments converge at a rate of $O(1/T)$ for 
$T$ samples, which is even faster than the $O(1/\sqrt{T})$ that would be obtained 
by sampling the exact posterior. Furthermore, Bach et al show that herding is 
equivalent to conditional gradient, and Husz\'{a}r and Duvenaud show that 
...
\citet{Welling:2009a,Gelfand:2010,Bornn:2013}.
...
gradient \cite{TODO} and hence (via the relationship in \cite{TODO}) also 
...
$h$ and $R$ is given in Figure~\ref{fig:table}.
